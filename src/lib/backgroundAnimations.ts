...
/**
 * Generates SVG animation content for background overlays.
 * Returns raw SVG elements (no outer <svg> wrapper) to be injected into parent SVGs.
 * All animations loop perfectly using SVG SMIL animations.
 */
export function generateAnimationSvg(
...
/**
 * Wraps animation content in a full <svg> element for preview rendering.
 */
export function generateAnimationSvgElement(
  animation: BackgroundAnimation | null,
  width: number,
  height: number,
  padding = 0,
...
// Same visual as the Resend overlay but each line gently
// oscillates vertically with a unique amplitude and phase,
// creating a flowing distortion field.
// ============================================================
...
// behind the terminal. Clearly visible, soft pastel colors.
// ============================================================
function auroraAnimation(w: number, h: number, p: number): string {
  const cx = w / 2, cy = h / 2;
  // Blob size: large enough to be clearly visible behind terminal
